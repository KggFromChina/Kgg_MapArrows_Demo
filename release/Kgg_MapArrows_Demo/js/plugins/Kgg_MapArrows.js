 ...
 * ==========================================================
 * 功能：
 * 本插件为RPG游戏的地图场景显示一个箭头，箭头在预先设定好的边框上
 * 移动，指向一个事件或坐标。
 ...
 * 会影响存档文件，因为有给$gameSystem添加成员。所有写入的
 ...
 * @option 左上角(top_left)
 * @value top_left
 * @desc 锚点原点类型（二选一：图像中心、左上角）
 * @default top_left
 * 
 * @param 锚点X
 * @desc 图像中锚点的X坐标。若在0-1之间则为比例，若大于1则视为像素数。
 * @default 0.8
 *
 * @param 锚点Y
 * @desc 图像中锚点的Y坐标。若在0-1之间则为比例，若大于1则视为像素数。
 * @default 0.5
 * 
 * @param 渐变最远距离
 ...
 * 箭头样式类
 */
Kgg_MapArrows.ArrowStyle = function (name, imagePath, canRotate, anchorOrigin, anchorX, anchorY, gradientMaxDistance, gradientMinDistance, borderType, borderOrigin, topBound, bottomBound, leftBound, rightBound, arrowEventDistance, blendMode) {
    this.name = name;
    this.imagePath = imagePath;
    this.canRotate = canRotate;
    this.anchorOrigin = anchorOrigin;
    this.anchor = { "x": anchorX, "y": anchorY };
    this.gradientMaxDistance = gradientMaxDistance;
    this.gradientMinDistance = gradientMinDistance;
    ...
    this.arrowEventDistance = arrowEventDistance;
...
Kgg_MapArrows.arrowExist = function (name) {
    if (!!$gameSystem._kgg_MapArrows_arrowMap[name]) return true;
    return false;
};
...
/**
 * 获取起点在屏幕上的位置
 * @param {Kgg_MapArrows.ArrowStyle} arrowStyle 
 * @returns 起点在屏幕上的位置
 */
Kgg_MapArrows.StartingPoint.screenPoint = function (arrowStyle) {
    switch (Kgg_MapArrows.StartingPoint.type) {
        ...
        case Kgg_MapArrows.StartingPointType.PLAYER_SCREEN_POSITION: // 玩家位置
            ...
            // 注意，这里不break。如果$gamePlayer不存在，就往下运行，取左上角像素位置。
...
Scene_Map.prototype.start = function () {
    ...
    if (!Kgg_MapArrows.arrowStyleBorderReady) {
        Kgg_MapArrows.initAllBorder(); // 初始化ArrowStyle的border，因为初始化太早会导致取不到Graphics的属性。
...
// 容易混淆需要区分的是$gameTemp._kgg_MapArrows_arrowLayer，这个是全部
// 箭头所在的层。
// =================================================================
...
Kgg_MapArrows.Screen.updatePosition = function (activeArrow) {
    // 计算与椭圆交点
    // A是屏幕中心点，B是目标点，C是与椭圆的交点，F是最终确定的箭头位置。
...
Kgg_MapArrows.Caculation.getIntersectionPoint = function (rectangle, pointA, pointB) {
    ...
    var bottomIntersection = slope === null ? { x: pointAX, y: rectY + rectHeight } : slope === 0 ? null : { x: pointAX + (rectY + rectHeight - pointAY) / slope, y: rectY + rectHeight };
...
for (var i = 0; i < Kgg_MapArrows.arrowStyleList.length; i++) {
    if (Kgg_MapArrows.parameters["箭头-" + String(i + 1)] != undefined &&
        ...
        var arrowStyle = new Kgg_MapArrows.ArrowStyle(
            ...
            Number(arrowStyleData["混合模式"] || 30)                    // blendMode 混合模式（1 2 3 4）
        );
