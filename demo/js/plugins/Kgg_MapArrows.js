...
Kgg_MapArrows.ArrowStyle = function (name, imagePath, canRotate,
    anchorOrigin, anchorX, anchorY,
    startingPointType, startingPointX, startingPointY,
    gradientMaxDistance, gradientMinDistance,
    ...
    this.anchor = { "x": anchorX, "y": anchorY };               // anchorX anchorY 锚点（XY坐标偏移量）

    this.startingPointType = startingPointType;                 // startingPointType 起点类型
    this.startingPoint = { "x": startingPointX, "y": startingPointY }; // startingPointX startingPointX 起点（XY坐标偏移量）
...
for (var i = 0; i < Kgg_MapArrows.arrowStyleList.length; i++) {
    if (Kgg_MapArrows.parameters["箭头-" + String(i + 1)] != undefined &&
        ...
        var arrowStyle = new Kgg_MapArrows.ArrowStyle(
            ...
            Number(arrowStyleData["锚点Y"] || 0.5),                     // anchorY
            ...
            String(arrowStyleData["边框类型(BorderType)"]
                || Kgg_MapArrows.BorderType.RECTANGLE),                 // borderType 边框类型（二选一：矩形、椭圆形） BorderType
            String(arrowStyleData["边框原点类型(BorderOrigin)"]
                || Kgg_MapArrows.BorderOrigin.SCREEN_EDGES),            // borderOrigin 边框原点类型（三选一：屏幕四边、屏幕中心、左上角） BorderOrigin
            Number(arrowStyleData["上界(topBound)"] || 0),              // topBound 上界（number）相对原点的偏移量
            Number(arrowStyleData["下界(bottomBound)"] || 0),           // bottomBound 下界（number）相对原点的偏移量
            Number(arrowStyleData["左界(leftBound)"] || 0),             // leftBound 左界（number）相对原点的偏移量
